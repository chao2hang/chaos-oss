...
- **S3 网关增强**：与主服务共用 `/s3` 路径，配置有效桶后自动启用，并可选开启根路径挂载的独立端口（标准 S3 客户端推荐）；桶支持映射多个存储挂载路径并选择写入策略（任一成功 / 全部成功）；默认拒绝未认证访问（匿名访问需显式开启设置项），访问密钥支持只读、限定桶范围与 IP 白名单。
...
S3 网关提供两种入口，共用同一套桶配置与访问密钥：

```text
主端口路径： https://example.com/s3      （默认，配置有效桶后自动启用）
独立端口：   https://example.com:5246    （可选，在 data/config.json 中开启）
```
...
- 主端口路径 `/s3` 在进入签名校验前会被网关剥掉 `/s3` 前缀，即 SigV4 的规范 URI 只包含 `/桶名/对象`。而标准 S3 客户端（aws-sdk-go / boto3 / minio-go / rclone 等）按「所发即所签」对完整请求路径签名，直接把 Endpoint 配成 `https://example.com/s3` 会得到 `403 signature verification failed`。该入口适用于自行控制签名逻辑的客户端，或在反向代理层把 `/桶名/*` 重写为 `/s3/桶名/*`（如 Caddy `rewrite * /s3{uri}`）来兼容标准客户端的场景。
- 需要标准 S3 客户端直接接入时，推荐开启独立端口：在 `data/config.json` 中设置 `"s3": { "enable": true, "port": 5246 }` 并重启服务，网关会以根路径挂载到该端口，Endpoint 直接填 `https://example.com:5246`（无路径），签名路径与请求路径完全一致。也可以用反向代理把独立端口映射为子域名（如 `s3.example.com`），Endpoint 填子域名即可。

两个入口可同时启用；桶配置为空或无效时，`/s3` 会保持关闭状态。
